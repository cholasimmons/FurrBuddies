<script lang="ts">
    ...
    const fphone = field('phone', 260, [pattern(phoneRegEx)]);
    const fgender = field('gender', '');
    const faddress = field('address', '');
    const fdistrict = field('district', '');
    ...
    async function submitEditedForm(){
        ...
        try {
            ...
            const prefs = {
                ...
                address: $faddress.value !== undefined ? $faddress.value : null,
                district: $fdistrict.value ?? null,
                photoID: bucketResponse.$id ?? null
            };
...
<main>
    <div class=" bg-surface-100 dark:bg-slate-800 p-8 rounded-2xl w-96 md:w-[40rem]">
        <h3 class="title">Edit { splitNames(name)}'s Details</h3>
        ...
        <form on:submit|preventDefault={submitEditedForm}>
            <div class="my-6 grid grid-cols-3 gap-3">
                <div class="col-span-3 flex flex-col gap-2 items-center justify-center bg-surface-300 rounded-xl p-6">
                    <input type="file" id="uploader" bind:value={$ffile.value} style="display:none" on:change={onChangeHandler}/>
                    {#if photofile}
                ...
                <div class="col-span-3 md:col-span-2">
                    ...
                    <input type="tel" id="phoneNumber" inputmode="tel" on:input={validateNumericInput} bind:value={$fphone.value}>
                </div>
