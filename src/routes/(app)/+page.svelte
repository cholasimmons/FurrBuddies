...
*/
	...
	const carouselStories:any = [
		{ image: '../images/slider1.webp', title: 'Council to ban certain breeds', story: 'Description goes here', link: 'https://simmons.studio', isActive: true },
...
<main>
	...
	{#if browser && carouselStories.length>0}
		...
		<div class="max-w-xl mx-auto max-h-[14rem] rounded-b-3xl overflow-hidden">
		<Carousel autoplay autoplayDuration={4400} pauseOnFocus={false} dots={false} arrows={false} swiping={true}>
			{#each carouselStories as { image, title, story, link }}
				<a href={link}  target="_blank">
					<div class="w-full max-h-[14rem] relative">
						<img src={image} alt='' class="object-cover object-center">
						{#if title}
	...
	<section class="px-{data.padding}">
		...
		{:else}
			...
			<div class="my-7 flex justify-evenly gap-3">
				{#each dashboardItems as item, index}
					<div in:fade={{ duration:300, delay: 50*(index+1) }} title={ item.title }
						class="rounded-2xl border-2 border-surface-300 shadow-[0_0.5rem_0.5rem_rgba(0,0,0,0.24)] hover:shadow-none transition-opacity bg-surface-50 dark:bg-surface-900">
